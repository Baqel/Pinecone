...
import xyz.baqel.pinecone.events.custom.PacketBaqelMoveEvent;
import xyz.baqel.pinecone.events.system.EventManager;
import xyz.baqel.pinecone.utils.*;
import xyz.baqel.pinecone.utils.blockbox.CollisionAssessment;

import java.util.List;

public class MovementParser {

    public static boolean parse(WrappedInFlyingPacket packet, PlayersData data) {
        ...
        data.addPastLocation(new PineconeLocation(to));
        ...
        List<BoundingBox> box = Pinecone.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(to.getWorld(), data.boundingBox.grow(0.5f, 0.1f, 0.5f).subtract(0, 0.5f, 0, 0, 0, 0));
        ...
        Pinecone.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(to.getWorld(), data.boundingBox.subtract(0, 2, 0, 0, 0, 0)).forEach(bb -> assessment.addMaterial(BlockUtils.getBlock(bb.getMinimum().toLocation(data.player.getWorld()))));
        ...
        data.riptideTicks = Pinecone.getInstance().getBlockBoxManager().getBlockBox().isRiptiding(data.player) ? Math.min(40, data.riptideTicks + 1) : Math.max(0, data.riptideTicks - 1);
        ...
        if (packet.isLook() || packet.isPos()) {
            PacketBaqelMoveEvent event = new PacketBaqelMoveEvent(data.player, new PineconeLocation(data.movement.from), new PineconeLocation(to), packet.isGround(), data.movement.hasJumped);
    ...
    private static Location toGround(PlayersData data) {
