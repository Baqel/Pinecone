...
import xyz.baqel.pinecone.utils.math.*;
...
public class PlayersData {
    ...
    public PineconeLocation lastTo;
    public org.bukkit.util.Vector lastVelocityTaken = new org.bukkit.util.Vector(0, 0, 0);
    ...
    public PlayerTimer lastFlag = new PlayerTimer(this), lastAttack = new PlayerTimer(this), lastOnGround = new PlayerTimer(this),
            ...
            lastTeleport = new PlayerTimer(PlayersData.this), lastUselessDamage = new PlayerTimer(PlayersData.this), lastFlyChange = new PlayerTimer(this);
    public List<PineconeLocation> pastLocation = Lists.newArrayList();
    ...
    public float getViolations(Check check) {
        if (Pinecone.getInstance().getCheckManager().violations.containsKey(player.getUniqueId())) {
            List<Violation> violationList = Pinecone.getInstance().getCheckManager().violations.get(player.getUniqueId());
    ...
    public PineconeLocation getLocationByTime(long millis) {
    ...
    public void addPastLocation(PineconeLocation pineconeLocation) {
        if (pastLocation.size() >= 20) {
            pastLocation.remove(0);
        }

        pastLocation.add(pineconeLocation);
    }
    ...
    public class Timers {
        public PlayerTimer lastJump = new PlayerTimer(PlayersData.this);
    }
