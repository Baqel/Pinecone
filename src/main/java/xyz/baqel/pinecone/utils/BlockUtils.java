...
public class BlockUtils {
    ...
    public static Block getBlock(Location location) {
        if (location.getWorld().isChunkLoaded(location.getBlockX() >> 4, location.getBlockZ() >> 4)) {
            ...
            Bukkit.getScheduler().runTask(Pinecone.getInstance(), futureTask);
            try {
    ...
    public static List<BoundingBox> getBlockBoundingBox(Block block) {
        List<BoundingBox> boxes = Pinecone.getInstance().getBlockBoxManager().getBlockBox().getSpecificBox(block.getLocation());
    ...
    public static boolean isTool(ItemStack stack) {
        ...
        return name.contains("bow") || name.contains("axe") || name.contains("spade") || name.contains("shovel") || name.contains("shear") || name.contains("sword");
    }
